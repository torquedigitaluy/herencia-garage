...
- **Etapa 1:** Home (`app/page.tsx`) que compone secciones en `components/sections/` + `components/layout/`, con imágenes placeholder (`components/ui/Placeholder.tsx`). Navbar con menú hamburguesa móvil. El copy ya es real (no queda Lorem Ipsum); las fotos siguen siendo placeholders.
- **Etapa 2:** animación de scroll "Restauración por etapas" (sección `#restauracion`) — `components/scroll/CanvasScroll.tsx` (client): precarga 121 frames WebP, los dibuja en `<canvas>` atados al progreso de `ScrollTrigger` (pin a 100vh, scrub), overlay de 4 etapas (Ingreso al taller → Chequeo a Fondo → Reparación de Chapa → Imprimación y Pintura), y limpia el ScrollTrigger al desmontar (`gsap.context().revert()`). Respeta `prefers-reduced-motion` (frame final estático). Si la URL trae hash, re-scrollea al anchor tras armar el pin (el pin cambia la altura del documento y cancela el scroll del navegador).
...
- **Post-roadmap:** rebrand visual — acento amarillo automotriz (tokens `amarillo`/`amarillo-claro` en `@theme`; el rojo ya no existe), hero "HERENCIA GARAGE" y copy real en todas las secciones. Los anchors de navbar/footer/hero son `/#seccion` para funcionar desde rutas internas.
- **Post-roadmap:** páginas de detalle del portfolio — `app/portfolio/[slug]/page.tsx` (pública, ISR 60s): historia de llegada, problemas[], soluciones[], antes/después y galería (tabla `portfolio_fotos`, fotos en `portfolio/galeria/<id>/`; RLS: SELECT público, ALL admin). `portfolio_publico` extendida (slug único, anio, historia, problemas, soluciones, duracion; fotos ahora nullable → placeholders). Seed con 5 proyectos inventados. Admin: `/admin/portfolio/[id]` edita detalle + galería (`PortfolioDetalleForm`, `GaleriaForm`). Tipos/slugify en `lib/portfolio.ts`. Los anchors del navbar/footer son `/#seccion` para funcionar desde rutas internas. Spec: `docs/superpowers/specs/2026-07-10-portfolio-detalle-design.md`.
...
**Deploy:** producción en https://herencia-garage.vercel.app (Vercel, team `torque-digital`). `origin` = `torquedigitaluy/herencia-garage`; **cada push a `main` despliega automáticamente**. Env vars de Supabase cargadas en Vercel (Production). Próximos pasos posibles: fotos reales (taller y proyectos), secuencia de scroll optimizada para móvil.

**Credenciales de prueba:** user/pass de admin y cliente en `credenciales.local.md` (gitignored, junto con todo `*.local.md`). `HG_project.md` tiene el resumen narrativo completo del sitio (funcionalidades y flujos, apto no-técnicos).
...
2. ✅ **Animación de scroll** (hoy "Restauración por etapas") — `<CanvasScroll>` con GSAP `ScrollTrigger`, pinning a 100vh, secuencia de 121 frames WebP.
